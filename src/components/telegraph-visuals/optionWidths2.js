...
const toggleData = (checked) => {
    let original_data = [...optionWidthsData2];
    let data_copy;
...
const OptionWidthViz2 = () => {
    ...
    const ToggleCheckbox = ({ checked2 }) => {
        return (
            <span className='toggle-checkbox'>
                ...
                <input type="checkbox" id="combine-widths-cb2" value="Sort Widths" checked={checked2} onChange={() => { setChecked2(!checked2) }}></input>
            </span>
        )
    };
    ...
    useEffect(() => {
        ...
        svg.append("text")
            ...
            .attr("y", config.h - config.paddingBot * (isMobile ? .8 : .6))
        ...
        svg.append("text")
            ...
            .attr("y", config.h - config.paddingBot * (isMobile ? .1 : .4))
            .attr("font-size", config.fontSize)
